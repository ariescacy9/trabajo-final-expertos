...
variables <- set(
  horas_trabajo = fuzzy_partition(varnames = c(poco = 180, regular = 360, mucho = 540),sd = 50.0),
  horas_estudio = fuzzy_partition(varnames = c(poco = 180, regular = 360, mucho = 480), sd = 40.0),
...
ejemplo.1 <- fuzzy_inference(relax_expert, list(horas_trabajo = 240, horas_estudio = 300, horas_deporte = 60))
# Ahora, defuizamos el ejemplo para transformar los parámetros en un número real
gset_defuzzify(ejemplo.1, "centroid")
# Mostramos el gráfico
plot(ejemplo.1)

ejemplo.2 <- fuzzy_inference(relax_expert, list(horas_trabajo = 490, horas_estudio = 0, horas_deporte = 240))
gset_defuzzify(ejemplo.2, "centroid")
plot(ejemplo.2)

ejemplo.3 <- fuzzy_inference(relax_expert, list(horas_trabajo = 0, horas_estudio = 546, horas_deporte = 120))
